...
public class LinearSearchStrategy implements SearchStrategy {
	...
	public int getProgress() { return _unitsDone; }

	@Override
	public int wrappedFind(TextFile src, String target, int start,
			boolean isCaseSensitive, boolean isWholeWord) {

		// search towards end of doc first...
		int foundOffset = find(src, target, start, src.getCurrentPageText().length(), isCaseSensitive, isWholeWord);
		// ...then from beginning of doc
		if (foundOffset < 0) {
			foundOffset = find(src, target, 0, start, isCaseSensitive, isWholeWord);
		}
	...
	@Override
	public int find(TextFile src, String target, int start, int end,
			boolean isCaseSensitive, boolean isWholeWord) {
		if (target.length() == 0) {
			return -1;
		}
		...
		if (end > src.getCurrentPageText().length()) {
			TextWarriorException.fail("TextBuffer.find: Invalid end position");
			end = src.getCurrentPageText().length();
		}

		end = Math.min(end, src.getCurrentPageText().length() - target.length() + 1);
		int offset = start;
		while (offset < end) {
	...
	@Override
	public int wrappedFindBackwards(TextFile src, String target, int start,
			boolean isCaseSensitive, boolean isWholeWord) {

		// search towards beginning of doc first...
		int foundOffset = findBackwards(src, target, start, -1, isCaseSensitive, isWholeWord);
		// ...then from end of doc
		if (foundOffset < 0) {
			foundOffset = findBackwards(src, target, src.getCurrentPageText().length() - 1, start, isCaseSensitive, isWholeWord);
		}
	...
	@Override
	public int findBackwards(TextFile src, String target, int start, int end,
			boolean isCaseSensitive, boolean isWholeWord) {
		if (target.length() == 0) {
			return -1;
		}
		if (start >= src.getCurrentPageText().length()) {
			TextWarriorException.fail("Invalid start position given to TextBuffer.find");
			start = src.getCurrentPageText().length() - 1;
		}
		if (end < -1) {
			TextWarriorException.fail("Invalid end position given to TextBuffer.find");
			end = -1;
		}
		int offset = Math.min(start, src.getCurrentPageText().length() - target.length());
		while (offset > end) {
			if (equals(src, target, offset, isCaseSensitive) &&
					(!isWholeWord || isSandwichedByWhitespace(src, offset, target.length()))) {
				break;
	...
	@Override
	public Pair replaceAll(TextFile src, String searchText,
		...
		int foundIndex = find(src, searchText, 0, src.getCurrentPageText().length(),
	...
	protected boolean equals(TextFile src, String target,
	 ...
	 * by whitespace.
	 *
	protected boolean isSandwichedByWhitespace(TextFile src,
			int start, int length) {
		boolean startWithWhitespace = (start == 0) || charSet.isWhitespace(src.charAt(start - 1));

		int end = start + length;
		boolean endWithWhitespace = (end == src.getCurrentPageText().length()) || charSet.isWhitespace(src.charAt(end));
